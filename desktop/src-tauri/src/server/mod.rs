...
pub fn start_server_internal(state: &State<AppState>, _app: &AppHandle) -> Result<(), String> {
    ...
    log::info!("Checking if server is already running on port {}", state.server_port);
    ...
    let server_dir = match find_server_directory() {
        Ok(dir) => {
            log::info!("Found server directory: {:?}", dir);
            dir
        }
        Err(e) => {
            log::error!("Server directory not found: {}", e);
            return Err(e);
        }
    };
    ...
    if !server_binary.exists() {
        ...
        if let Err(e) = build_go_server(&server_dir) {
            log::error!("Failed to build server: {}", e);
            return Err(e);
        }
    ...
    match cmd.spawn() {
        Ok(child) => {
            ...
            *server_process = Some(child);
            ...
            log::info!("Server started with PID: {}", child_id);
    ...
    for path in &possible_paths {
        ...
        if path.exists() {
            ...
            if path.join("go.mod").exists() {
                log::info!("Found Go server directory with go.mod at: {:?}", path);
                return Ok(path.clone());
            }
            // Check for pre-built Bun executable
            else if path.join("tunnelforge").exists() {
                log::info!("Found pre-built server executable at: {:?}", path);
                return Ok(path.clone());
            }
            else {
                log::debug!("Directory exists but no server found: {:?}", path);
            }
        } else {
            log::debug!("Directory does not exist: {:?}", path);
        }
    }
...
pub async fn get_server_status(state: State<'_, AppState>) -> Result<ServerStatus, String> {
    ...
    let status = match &*server_process {
        Some(child) => {
            ...
            log::debug!("Server process PID {} running: {}", child.id(), running);
...
pub async fn get_server_url(state: State<'_, AppState>) -> Result<String, String> {
    ...
    log::debug!("Returning server URL: {}", url);
    Ok(url)
}
...
pub async fn open_web_interface(state: State<'_, AppState>) -> Result<(), String> {
    let url = format!("http://localhost:{}", state.server_port);
    log::info!("Opening web interface at: {}", url);
