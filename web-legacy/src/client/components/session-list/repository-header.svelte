...
<script lang="ts">
  ...
  interface Props {
    repoPath: string;
    followMode?: string;
    followModeSelector?: string;
    worktreeSelector?: string;
  }

  let { repoPath, followMode, followModeSelector, worktreeSelector }: Props = $props();

  function getRepoName(): string {
    return getBaseRepoName(repoPath);
  }

  $: cleanBranchName = followMode ? followMode.replace(/^refs\/heads\//, '') : null;
</script>
...
<div class="flex items-center justify-between mb-3">
  <div class="flex items-center gap-2">
    ...
    <h4 class="text-sm font-medium text-text-muted flex items-center gap-2">
      ...
      {#if cleanBranchName}
        <span
          class="text-[10px] px-1.5 py-0.5 bg-purple-500/20 text-purple-400 rounded flex items-center gap-1"
  ...
  <div class="flex items-center gap-2">
    {@html followModeSelector || ''}
    {@html worktreeSelector || ''}
  </div>
</div>