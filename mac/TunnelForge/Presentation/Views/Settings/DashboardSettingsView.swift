...
struct DashboardSettingsView: View {
    ...
    private var serverPort = "\(AppConstants.Defaults.serverPort)"
        ...
        Section {
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if let pid = serverManager.processIdentifier {
...
// Port conflict warning
    ...
    private func checkPortAvailability() async {
        ...
        if let conflict = await PortConflictResolver.shared.detectConflict(on: port) {
            // Only show warning for non-TunnelForge processes
            // TunnelForge instances will be auto-killed by ServerManager
