...
async function getBranchStats(
  ...
  try {
    ...
    if (match) {
      ...
      stats.insertions = Number.parseInt(match[2], 10) || 0;
      stats.deletions = Number.parseInt(match[3], 10) || 0;
    }
  } catch (error) {
...
export function createWorktreeRoutes(): Router {
  ...
  router.get('/worktrees', async (req, res) => {
    try {
      ...
      try {
        const { stdout } = await execGit(['config', 'tunnelforge.followWorktree'], {
  ...
  router.post('/worktrees/follow', async (req, res) => {
    try {
      ...
      if (enable) {
        ...
        await execGit(['config', '--local', 'tunnelforge.followWorktree', worktree.path], {
          cwd: absoluteRepoPath,
        });
