<script lang="ts">
  ...
  let { visible = false, items = [], selectedIndex = -1, isLoading = false } = $props();

  const dispatch = createEventDispatcher<{
    'item-selected': { suggestion: string };
  }>();

  function handleItemClick(item: AutocompleteItem) {
    dispatch('item-selected', { suggestion: item.suggestion });
  }
</script>
    ...
    <div class="max-h-48 sm:max-h-64 lg:max-h-80 overflow-y-auto">
      {#each items as item, index}
        <button
          ...
          class="w-full text-left px-3 py-2 hover:bg-surface-hover transition-colors duration-200 flex items-center gap-2 {index === selectedIndex ? 'bg-primary/20 border-l-2 border-primary' : ''}"
          type="button"
        >
          <svg
            width="12"
          ...
          {#if item.gitBranch}
            <span class="text-primary text-[9px] sm:text-[10px] flex items-center gap-1">
              ...
              {#if item.isWorktree}
                <span class="text-purple-500 ml-0.5" title="Git worktree">
          ...
          {#if item.gitAddedCount || item.gitModifiedCount || item.gitDeletedCount}
            <div class="flex items-center gap-1.5 text-[9px] sm:text-[10px]">
              {#if item.gitAddedCount && item.gitAddedCount > 0}
              ...
              {/if}
              {#if item.gitModifiedCount && item.gitModifiedCount > 0}
              ...
              {/if}
              {#if item.gitDeletedCount && item.gitDeletedCount > 0}
