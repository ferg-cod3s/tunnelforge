...
export class AuthClient {
  private static readonly TOKEN_KEY = 'tunnelforge_auth_token';
  private static readonly USER_KEY = 'tunnelforge_user_data';
  ...
  /**
   * Set current user for no-auth mode
   */
  setNoAuthUser(userId: string): void {
    this.setCurrentUser({
      userId,
      token: 'no-auth-token',
      authMethod: 'password', // Use password as fallback auth method
      loginTime: Date.now(),
    });
    logger.log('👤 No-auth user set:', this.getCurrentUser());
  }
