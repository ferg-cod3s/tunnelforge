...
test.describe('TunnelForge Mac App - Error Scenarios & Edge Cases', () => {
  ...
  test('should handle network disconnection gracefully', async ({ page }) => {
    ...
    const firstSession = page
      ...
      .or(page.locator('.session-card'))
      .first();
    await firstSession.click();

    // Wait for terminal to load
    const terminal = page.locator('vibe-terminal').or(page.locator('vibe-terminal-binary')).first();
  ...
  test('should handle empty session lists', async ({ page }) => {
    ...
    const emptyStateMessages = page
      .locator('text=No sessions')
      .or(page.locator('text=Create your first').or(page.locator('text=Get started')));
  ...
  test('should handle rapid session creation', async ({ page }) => {
    ...
    const sessionCards = page.locator('[data-testid*="session"]').or(page.locator('.session-card'));
...
test.describe('TunnelForge Mac App - Visual Verification', () => {
  ...
  test('should display complete UI elements', async ({ page }) => {
    ...
    const uiChecklist = [
      ...
      {
        selector: '[data-testid="create-session"]',
        description: 'Create session button',
        fallback: 'button:has-text("+")',
      },

      // Session list area
      {
        selector: '[data-testid="session-list"]',
        description: 'Session list container',
      ...
      // Theme toggle
      {
        ...
        fallback: '[data-testid="theme-toggle"]',
      },
      ...
      // Settings or menu
      {
        ...
        fallback: 'button[title*="settings"]',
      },
    ];
  ...
  test('should be responsive and handle window resizing', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      if (bodyWidth > windowWidth + 10) {
        ...
        console.log(
          `⚠️  Horizontal scroll detected at ${viewport.name}: body=${bodyWidth}px, window=${windowWidth}px`
        );
      }
    }
  });
